...
export default function CVContent() {
  ...
  return (
    <div className="min-h-screen bg-zinc-100 p-4 md:p-8 flex flex-col items-center gap-8 print:p-0 print:bg-white print:block">
      {/* Print Button */}
      <div className="print-hidden fixed top-4 right-4 md:bottom-8 md:right-8 md:top-auto z-50 flex gap-2 md:gap-4">
        <Link href="/">
        ...
        <header className="border-b-4 border-black pb-4 mb-4 flex justify-between items-start gap-6">
          <div className="flex-1 flex flex-col justify-between h-full">
            <div>
              ...
              <p className="text-xs text-zinc-700 max-w-md text-justify font-medium leading-relaxed border-l-4 border-zinc-200 pl-3">
                {DATA.about}
              </p>
            </div>
                ...
                {DATA.experience.slice(0, 4).map((role, index) => (
                  <div key={index} className="relative pl-4 border-l-2 border-zinc-300">
                    <div className="flex justify-between items-baseline mb-1">
                      ...
                      <span className="text-[11px] font-bold border border-black px-1.5 bg-zinc-50">{role.period}</span>
                    </div>
                    <p className="text-xs font-bold text-zinc-800 mb-1">{role.role}</p>
                    <p className="text-[11px] text-zinc-700 text-justify leading-snug">{role.description}</p>
                  </div>
                ))}
              </div>
            </section>
                  ...
                  <div key={project.id} className="border-2 border-black p-2.5 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] bg-white">
                    <div className="flex justify-between items-start mb-1">
                        <div>
                            ...
                            <p className="text-[11px] text-zinc-700 mb-1.5 line-clamp-2">{project.desc}</p>
                            <div className="flex flex-wrap gap-1 mb-1">
                                {project.tech.map((t, i) => (
                                    <span key={i} className="text-[10px] font-bold border border-black px-1 bg-zinc-100">{t}</span>
                                ))}
                            </div>
                        </div>
            ...
            <section>
              ...
              <div className="flex flex-wrap gap-1.5">
                {DATA.skills.map((skill, index) => (
                  <span key={index} className="text-[11px] font-bold border-2 border-black px-2 py-0.5 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] bg-white">
                    {skill}
                  </span>
                ))}
              </div>
            ...
            <section>
              ...
              <div className="space-y-2">
                <div className="bg-zinc-50 p-1.5 border border-black">
                  ...
                  <p className="text-[11px] text-zinc-700 leading-tight">{DATA.technicalSkills.frontend.join(" • ")}</p>
                </div>
                <div className="bg-zinc-50 p-1.5 border border-black">
                  <h3 className="text-xs font-black mb-0.5 uppercase">Backend</h3>
                  <p className="text-[11px] text-zinc-700 leading-tight">{DATA.technicalSkills.backend.join(" • ")}</p>
                </div>
                <div className="bg-zinc-50 p-1.5 border border-black">
                  <h3 className="text-xs font-black mb-0.5 uppercase">Outils</h3>
                  <p className="text-[11px] text-zinc-700 leading-tight">{DATA.technicalSkills.tools.join(" • ")}</p>
                </div>
              </div>
            </section>
              ...
              <div className="space-y-2">
                {DATA.education.map((edu, index) => (
                  <div key={index} className="group">
                    ...
                    <p className="text-[11px] text-zinc-700 italic">{edu.degree}</p>
                    <p className="text-[11px] font-bold bg-black text-white inline-block px-1 mt-0.5">{edu.period}</p>
                  </div>
                ))}
              </div>
            </section>
